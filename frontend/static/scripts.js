...
function produceNewspaper() {
  ...
  // fetch('http://43.207.194.38:8000/generate_article', {
  //     method: 'POST',
  //     headers: {
  ...
  //     });
  fetch("http://localhost:8000/generate_article", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      'Access-Control-Allow-Origin': '*',
...
function toggleLoading(isLoading) {
  ...
  const messages = [
    ...
    "Curating sources...",
    "Writing articles...",
    "Editing final result...",
  ];
  loadingMessages.style.fontFamily = "'Bodoni Moda', sans-serif";
  if (isLoading) {
...
window.addEventListener("DOMContentLoaded", (event) => {
  document
    .getElementById("produceNewspaper")
    .addEventListener("click", produceNewspaper);
  document.querySelectorAll(".layout-icon").forEach((icon) => {
